...
std::ostream& operator << (std::ostream &out, const std::shared_ptr < KoreanCompoundType >  &compound) {
		out << compound->initial->begining << compound->middle->firstlast;
		if(compound->last) {
			out << compound->last->end;
		}
...
std::shared_ptr < KoreanCompoundType > getCompound(std::string target) {
	for(int i = 0; i < compounds.size(); i++) {
		if(compounds[i]->compoundChar == target) {
			return compounds[i];
		}
	}
	return nullptr;
	//std::cout << "INVALID compound PARSE: " << target << std::endl;
}
